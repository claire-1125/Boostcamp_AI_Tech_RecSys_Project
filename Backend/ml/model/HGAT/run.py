...
def run():
    ...
    user_rel_matrix, recipe_rel_matrix = torch.load('./recipe_Data/data/1.pt'),[  
                                                                            torch.load('./recipe_Data/data/2.pt'), 
                                                                            torch.load('./recipe_Data/data/3.pt'), 
                                                                            torch.load('./recipe_Data/data/4.pt')]
    ...
    if args.load_label:
        label = torch.load('./recipe_Data/data/label.pt').to(args.device)
    else:        
        ...
        for u in tqdm(train.유저_아이디.unique()):
            ...
            for r in test[test.유저_아이디==u].레시피_아이디.unique():
                r_idx = recipe2idx[r]
                label[u_idx][r_idx] = 1
        torch.save(label, './recipe_Data/data/label.pt') 
    ...
    for e in tqdm(range(epochs)):
        for b_idx in dataloader:
            ...
            if loss_train.detach().item() < best_loss:
                torch.save(model.state_dict(), './recipe_Data/model_save/best_model.pt')
